...
\begin{itemize}
    \item Specify the appropriate measures for central tendency and dispersion for each measurement level;
    \item Reproduce and understand the formulas for calculating the mean, variance, and standard deviation of a sample;
    ...
    \item Apply suitable visualization techniques for a given variable;
...
Imagine a group of researchers wants to conduct a study on superheroes. Questions they could ask include:
...
Table~\ref{tab:centrum-spreidingsmaten} provides an overview of suitable measures for central tendency and dispersion. These are defined later in this chapter.
...
\begin{itemize}
    ...
    \item a median and interquartile range.
\end{itemize}

Suppose the researchers carried out a number of measurements regarding the size of superheroes. The results can be found in Figure~\ref{fig:heroes-size}.
...
\begin{exercise}
    What is the average size of the superheroes in Figure~\ref{fig:heroes-size}?
\end{exercise}
...
\begin{exercise}
    ...
    Assume that Kabouter Wesley (10 cm) is added to the team of superheroes. What happens to the arithmetic mean?
\end{exercise}

\subsection{Variance and standard deviation}
\label{ssec:variance-and-standard-deviation}

The measure for dispersion that is typically associated with the average is the standard deviation.
...
\begin{example}
  In the manufacturing process of screw drivers, the size of the head is important for its functioning. When the sizes of the heads of a batch of 1000 screw drivers are measured, these should be very similar, so a very small standard deviation is an indication of good quality.    
\end{example}

\begin{example}
  Superheroes are a very diverse group. Some are very rich (e.g.~Batman, Iron Man), some are middle class (e.g.~Spiderman), others are quite poor (e.g.~Arsenal). The dispersion of their income would be very large, but that isn't necessarily a bad thing.
\end{example}
...
\begin{definition}[Median]
    The \emph{median}\index{median} is the value separating the higher half of a data set from the lower half. When you sort the values from low to high, the median will be the value in the middle. If the set has an even number of values, the median is the arithmetic mean of the two middle values. 
\end{definition}

\begin{exercise}
    What is the median of the sizes of our superheroes?
\end{exercise}
...
\begin{definition}[Quartiles]
    ...
    \begin{itemize}
        \item The first/lower quartile ($Q_{1}$) is the value that separates the lowest quarter of values from the others.
        \item The second quartile ($Q_{2}$) is the value that separates the lowest half of values from the highest half.
        \item The third/higher quartile ($Q_{3}$) is the value that separates the highest quarter of values from the others.
    \end{itemize}
\end{definition}
...
A \index{box plot}\emph{box plot} is a chart that shows the dispersion of a set of values (cfr. Figure~\ref{fig:boxplot}). It consists of a rectangle with sides at the quartiles ($Q_1$ and $Q_3$). Inside the rectangle, the median is also plotted. The whiskers attached to the upper and lower sides of the rectangle cover the remaining observations, except for outliers and extremes.
...
As you might have noticed, these metrics are also used in the research of~\textcite{Akin2016}.
In the next exercises, we will try to reproduce the results.
...
After importing the file, try to calculate the average execution time, the standard deviation, the different quartiles, etc.
Use the commands \texttt{mean}, \texttt {median}, \texttt {quantile}, \texttt {min}, \texttt{max}, \texttt{var}, \texttt{sd}. 
You can also use the \texttt{summary} command to get a nice overview.
...
A boxplot shows the median, quartiles, and extreme values of a dataset. 
...
In the previous exercises, when we tried to analyze the dataset as a whole, we noticed that this is pretty much useless, since the dataset is divided into several categories. 
...
\begin{exercise}[Retrieval practice]
  Use the procedure for retrieval practice from Exercise~\ref{ex:retrieval-practice-measurement-levels} 
  to study the \emph{techniques for analysis and visualization of a single variable}.
  
  Provide for each measurement level:
  
  \begin{itemize}
    \item The appropriate measures for central tendency and dispersion (name + definitions and formulas)
